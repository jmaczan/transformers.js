...
} from "./generation/stopping_criteria.js";
...
async function getSession(pretrained_model_name_or_path, fileName, options) {
  ...
  if (typeof dtype !== "string") {
    if (dtype && dtype.hasOwnProperty(fileName)) {
      ...
      console.warn(
        `dtype not specified for "${fileName}". Using the default dtype for this device (${dtype}).`
      );
    }
  }
  ...
  if (!DEFAULT_DTYPE_SUFFIX_MAPPING.hasOwnProperty(dtype)) {
    throw new Error(
      `Invalid dtype: ${dtype}. Should be one of: ${Object.keys(
        DATA_TYPES
      ).join(", ")}`
    );
  } else if (
    dtype === DATA_TYPES.fp16 &&
    device === "webgpu" &&
  ...
  const bufferPromise = getModelFile(
  ...
  if (
    options.use_external_data_format &&
  ...
  return Object.fromEntries(
    await Promise.all(
      Object.keys(names).map(async (name) => {
        const { buffer, session_options } = await getSession(
          pretrained_model_name_or_path,
          names[name],
          options
        );
...
async function sessionRun(session, inputs) {
  ...
  try {
    ...
    const ortFeed = Object.fromEntries(
      Object.entries(checkedInputs).map(([k, v]) => [k, v.ort_tensor])
    );
...
async function seq2seqForward(self, model_inputs) {
  ...
  // Encode if needed
  if (!encoder_outputs) {
    const encoder_inputs = pick(
      model_inputs,
      self.sessions["model"].inputNames
    );
    ...
    if (pixel_values && input_ids.dims[1] !== 1) {
      ...
      attention_mask = cat(
        [
          ...
          attention_mask.slice(null, [
            attention_mask.dims[1] - target_length,
...
function decoder_prepare_inputs_for_generation(
  ...
  if (model_inputs.past_key_values) {
    const past_length = Object.values(model_inputs.past_key_values)[0].dims.at(
      -2
    );
    ...
    // 3 - Otherwise (past_length >= input_ids.shape[1]), let's assume input_ids only has unprocessed tokens.
      ...
      if (
        ...
        const num_new_tokens =
          input_ids.dims[1] - (past_length - num_image_tokens);
...
export class PreTrainedModel extends Callable {
  ...
  _get_logits_processor(
    ...
    if (
      ...
      processors.push(
        new NoRepeatNGramLogitsProcessor(generation_config.no_repeat_ngram_size)
      );
    }
  ...
  _validate_model_class() {
    if (!this.can_generate) {
      ...
      if (generate_compatible_classes.size > 0) {
        errorMessage += ` Please use the following class instead: ${[
  ...
  _update_model_kwargs_for_generation({
    ...
    model_inputs["input_ids"] = new Tensor(
      "int64",
      generated_input_ids.flat(),
  ...
  async _prepare_encoder_decoder_kwargs_for_generation({
    ...
    if (
      ...
      const { input_ids, pixel_values, attention_mask, ...kwargs } =
        model_inputs;
      // @ts-ignore
    ...
    while (true) {
      ...
      if (stop.every((x) => x)) {
        if (generation_config.return_dict_in_generate) {
          ...
          past_key_values = this.getPastKeyValues(
            outputs,
            model_inputs.past_key_values,
            false
          );
...
export class LlavaForConditionalGeneration extends LlavaPreTrainedModel {
  _merge_input_ids_with_image_features({
    ...
    const indexOfImage = idsList.map((x) =>
      x.findIndex((x) => x == image_token_index)
    );
    ...
    for (let i = 0; i < indexOfImage.length; ++i) {
      ...
      stacked.push(
        cat([e.slice([0, index]), im, e.slice([index + 1, e.dims[0]])], 0)
      );
...
export class Florence2ForConditionalGeneration extends Florence2PreTrainedModel {
  ...
  async forward({
    ...
    if (!encoder_outputs) {
      ...
      let { last_hidden_state } = await encoderForward(this, {
        inputs_embeds,
        attention_mask,
      });
    ...
    if (!decoder_inputs_embeds) {
      ...
      decoder_inputs_embeds = await this.encode_text({
...
export class SamModel extends SamPreTrainedModel {
  ...
  async forward(model_inputs) {
    ...
    return await sessionRun(
      this.sessions["prompt_encoder_mask_decoder"],
      decoder_inputs
    );
  }
  ...
  async generate_speech(
    ...
    while (true) {
      ...
      decoder_outputs = await sessionRun(
        this.sessions["decoder_model_merged"],
        decoderFeeds
      );
    ...
    const { waveform } = await sessionRun(vocoder.sessions["model"], {
      spectrogram,
    });
...
export class MusicgenForConditionalGeneration extends PreTrainedModel {
  ...
  _apply_and_filter_by_delay_pattern_mask(outputs) {
    ...
    return new Tensor(outputs.type, outputs.data.slice(0, newDataSize), [
      batch_size,
      num_codebooks,
      inferred,
    ]);
  ...
  prepare_inputs_for_generation(input_ids, model_inputs, generation_config) {
    ...
    const prepped = super.prepare_inputs_for_generation(
      clonedInputIds,
      model_inputs,
      generation_config
    );
  ...
  async generate(options) {
    ...
    const { audio_values } = await sessionRun(this.sessions["encodec_decode"], {
      audio_codes,
    });
...
export class CausalLMOutput extends ModelOutput {
   ...
   * //@param {Tensor} output.logits Prediction scores of the language modeling head (scores for each vocabulary token before softmax).
   */
  constructor({ logits, cpuData, ...rest }) {
    super();
